...
void Precompute::getRandomBit(vector<smallType> &a, size_t size)
{
	assert(a.size() == size && "size mismatch for getRandomBitShares");
	std::fill(a.begin(), a.end(), 0);
}

void Precompute::getRandomValue(vector<smallType> &a, size_t size)
{
	assert(a.size() == size && "size mismatch for getRandomBitShares");
	std::fill(a.begin(), a.end(), 0);
}

void Precompute::getxyz(vector<smallType> &x, vector<smallType> &y, vector<smallType> &z, size_t size)
{
	assert(x.size() == size && "size mismatch for getRandomBitShares");
	assert(y.size() == size && "size mismatch for getRandomBitShares");
	assert(z.size() == size && "size mismatch for getRandomBitShares");
